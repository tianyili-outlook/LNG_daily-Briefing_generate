...
def change_price_describe(document, new_cache_grid):
    def _price_describe_helper(contract_type):
        ...
        elif new_cache_grid.iloc[-1][contract_type + ' 月份'] != new_cache_grid.iloc[-1][contract_type + ' 月份']:
    ...
    # if new_cache_grid.iloc[-1]['HH 价格'] == 0:
    #     txt = '北美市场：昨日美国假期，纽约商品交易所（NYMEX）HH期货合约无报价。'
    # elif new_cache_grid.iloc[-1]['HH 月份'] != new_cache_grid.iloc[-1]['HH 月份']:
    #     txt = '北美市场：纽约商品交易所（NYMEX）HH期货合约换至{:.0f}月，收于{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['HH 月份'], new_cache_grid.iloc[-1]['HH 价格'])
    # elif new_cache_grid.iloc[-2]['HH 价格'] == 0:
    #     txt = '北美市场：纽约商品交易所（NYMEX）HH期货合约恢复报价，收于{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['HH 价格'])
    # else:
    #     price = new_cache_grid.iloc[-1]['HH 价格']
    #     diff = price - new_cache_grid.iloc[-2]['HH 价格']
    #     change = '上涨' if diff > 0 else '下跌'
    #     percent = diff / new_cache_grid.iloc[-2]['HH 价格'] * 100
    #     txt = '北美市场：纽约商品交易所（NYMEX）HH期货合约收于{:.3f}美元/百万英热单位，较前一交易日{}{:.3f}美元（{:.2f}%）。'.format(price, change, abs(diff), percent)

    # if new_cache_grid.iloc[-1]['NBP 价格'] == 0:
    #     txt += '英国市场：昨日英国假期，伦敦洲际交易所（ICE）NBP期货合约无报价。'
    # elif new_cache_grid.iloc[-1]['NBP 月份'] != new_cache_grid.iloc[-1]['NBP 月份']:
    #     txt += '英国市场：伦敦洲际交易所（ICE）NBP期货合约换至{:.0f}月，收于{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['NBP 月份'], new_cache_grid.iloc[-1]['NBP 价格'])
    # elif new_cache_grid.iloc[-2]['NBP 价格'] == 0:
    #     txt += '英国市场：伦敦洲际交易所（ICE）NBP期货合约恢复报价，收于{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['NBP 价格'])
    # else:
    #     price = new_cache_grid.iloc[-1]['NBP 价格']
    #     diff = price - new_cache_grid.iloc[-2]['NBP 价格']
    #     change = '上涨' if diff > 0 else '下跌'
    #     percent = diff / new_cache_grid.iloc[-2]['NBP 价格'] * 100
    #     txt += '英国市场：伦敦洲际交易所（ICE）NBP期货合约收于{:.3f}美元/百万英热单位，较前一交易日{}{:.3f}美元（{:.2f}%）。'.format(price, change, abs(diff), percent)

    # if new_cache_grid.iloc[-1]['JKM 价格'] == 0:
    #     txt += '日韩市场：昨日新加坡假期，普氏日韩LNG现货（JKM）无评估。'
    # elif new_cache_grid.iloc[-1]['JKM 月份'] != new_cache_grid.iloc[-1]['JKM 月份']:
    #     txt += '日韩市场：普氏日韩LNG现货（JKM）换至{:.0f}月，首日评估{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['JKM 月份'], new_cache_grid.iloc[-1]['JKM 价格'])
    # elif new_cache_grid.iloc[-2]['JKM 价格'] == 0:
    #     txt += '日韩市场：普氏日韩LNG现货（JKM）恢复评估，价报{:.3f}美元/百万英热单位。'.format(new_cache_grid.iloc[-1]['JKM 价格'])
    # else:
    #     price = new_cache_grid.iloc[-1]['JKM 价格']
    #     diff = price - new_cache_grid.iloc[-2]['JKM 价格']
    #     change = '上涨' if diff > 0 else '下跌'
    #     percent = diff / new_cache_grid.iloc[-2]['JKM 价格'] * 100
    #     txt += '日韩市场：普氏日韩LNG现货（JKM）到岸评估价报{:.3f}美元/百万英热单位，较前一交易日{}{:.3f}美元（{:.2f}%）。'.format(price, change, abs(diff), percent)
...
def change_table2(document, pipeline_data, today):
    ...
    temp.runs[1].text = today[4:6].lstrip('0')
    temp.runs[3].text = today[6:].lstrip('0')
    temp.runs[9].text = yesterday[4:6].lstrip('0')
    temp.runs[11].text = yesterday[6:].lstrip('0')
    temp.runs[18].text = today[4:6].lstrip('0')
    temp.runs[20].text = today[6:].lstrip('0')
    for j in [4, 6, 9]:
        temp = document.tables[1].rows[1].cells[j].paragraphs[0].runs[0]
        temp.text = today[4:6].lstrip('0') + temp.text
...
with col2:
    st.download_button(label = '下载', 
        data = target, 
        file_name = '每日天然气简报' 
                    + st.session_state.today[:4] 
                    + '.' 
                    +  st.session_state.today[4:6]
                    + '.'
                    + st.session_state.today[6:]
                    + '.docx')
